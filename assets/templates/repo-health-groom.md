...
safe-outputs:
  ...
  hide-comment: 50
triggers:
  - schedule: "{{GROOM_SCHEDULE}}"
  - workflow_dispatch
---

# Repo Health — Dashboard Groomer

Maintain the health dashboard issue for **{{REPO_OWNER}}/{{REPO_NAME}}** — link investigation results, mark resolved findings, hide stale comments, and keep the dashboard clean.
...
# Get issue body
BODY=$(gh issue view "$ISSUE" --repo {{REPO_OWNER}}/{{REPO_NAME}} --json body --jq '.body')

# Get all comments
COMMENTS=$(gh issue view "$ISSUE" --repo {{REPO_OWNER}}/{{REPO_NAME}} \
...
gh issue edit "$ISSUE" --repo {{REPO_OWNER}}/{{REPO_NAME}} --body "$UPDATED_BODY"
```
