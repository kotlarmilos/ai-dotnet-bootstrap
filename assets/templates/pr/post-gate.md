...
Each AI model has different strengths — one may spot a root cause another misses, or propose a simpler fix. By running try-fix with {{MODEL_COUNT}} models sequentially, you maximize fix diversity and increase the chance of finding the optimal solution.
...
Run the `try-fix` skill **{{MODEL_COUNT}} times sequentially**, once with each model (see `SHARED-RULES.md` for model list).
...
|---|-------|----------|--------|---------------|-------|
| 1 | {{MODEL_1}} | [approach] | ✅/❌ | `file.cs` | [why] |
| 2 | {{MODEL_2}} | [approach] | ✅/❌ | `file.cs` | [why] |
