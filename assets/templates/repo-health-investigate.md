...
gh issue view ISSUE_NUMBER --repo OWNER/REPO --json title,body,comments,labels,assignees,milestone,createdAt,updatedAt

# Check for related PRs
gh pr list --repo OWNER/REPO --search "ISSUE_NUMBER" --json number,title,state,url

# Check if this is a regression — search for related closed issues
gh issue list --repo OWNER/REPO --state closed --search "KEYWORDS" --json number,title,closedAt --limit 10

# Check recent timeline
gh api repos/OWNER/REPO/issues/ISSUE_NUMBER/timeline --jq '.[].event' | sort | uniq -c
```
...
```bash
# Get full PR details
gh pr view PR_NUMBER --repo OWNER/REPO --json title,body,author,reviewDecision,reviewRequests,statusCheckRollup,files,comments,createdAt,updatedAt

# Check CI status details
gh pr checks PR_NUMBER --repo OWNER/REPO

# Get CI failure logs (if failing)
FAILED_RUN=$(gh run list --repo OWNER/REPO --branch PR_BRANCH --json databaseId,conclusion --jq '[.[] | select(.conclusion == "failure")][0].databaseId')
if [ -n "$FAILED_RUN" ]; then
  gh run view "$FAILED_RUN" --repo OWNER/REPO --log-failed 2>/dev/null | tail -100
fi

# Check review comments for blockers
gh pr view PR_NUMBER --repo OWNER/REPO --json reviewRequests,latestReviews --jq '.latestReviews[] | {author: .author.login, state: .state, body: .body}'
```
...
gh run list --repo OWNER/REPO --workflow "WORKFLOW_NAME" --branch main --json databaseId,conclusion,createdAt,event --limit 10

# Get failed run logs
FAILED_RUN=$(gh run list --repo OWNER/REPO --workflow "WORKFLOW_NAME" --branch main --json databaseId,conclusion --jq '[.[] | select(.conclusion == "failure")][0].databaseId')
gh run view "$FAILED_RUN" --repo OWNER/REPO --log-failed 2>/dev/null | tail -200

# Compare with last green run
GREEN_RUN=$(gh run list --repo OWNER/REPO --workflow "WORKFLOW_NAME" --branch main --json databaseId,conclusion --jq '[.[] | select(.conclusion == "success")][0].databaseId')
gh run view "$GREEN_RUN" --repo OWNER/REPO --json jobs --jq '.jobs[] | {name, conclusion, steps: [.steps[] | select(.conclusion == "failure") | {name, conclusion}]}'

# Check for infrastructure vs code issues
gh run view "$FAILED_RUN" --repo OWNER/REPO --log-failed 2>/dev/null | grep -iE "timeout|oom|disk.space|rate.limit|connection" | head -10
```
...
```bash
gh issue comment ${{ inputs.health_issue_number }} --repo OWNER/REPO --body "$REPORT"
```
